...
- The approved Gemma worker stays resident after a successful request. The chat header reports unloaded, working, or ready state, shows subtle live VRAM with context beneath it beside the loaded model name, and offers an idle-only typed unload action; unload and shutdown reclaim the complete native worker process.
...
- The Windows full-VRAM policy and the dedicated `pnpm test:m3:windows:memory` canary are implemented. The canary requires the applied generation cap to equal separately reported detected GPU VRAM, actual GPU allocation not to exceed that cap, automatic context to exceed the former 8K setting, and clean shutdown; it has not yet run on physical Windows hardware.
