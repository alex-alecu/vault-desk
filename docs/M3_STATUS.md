...
- Product surface: New chat, immutable file attachments, folder groups, recent persistent sessions, drafts, durable messages and normalized execution records, warm-session restoration, cancellation, artifacts, and Overview-first Technical details with live logs.
...
- The Tauri and React desktop retains native folder and file selection, grouped sessions, newest-five pagination, drafts, conversation restoration, task activity, generated files, cancellation, keyboard and responsive behavior. Technical details opens to Overview without log bodies; Logs contains collapsed execution rows with selectable Output, Errors, and typed VM diagnostics, explicit byte/truncation status, near-bottom following, and Jump to latest. The webview receives opaque identifiers and no new authority.
- Vault Core persists schema-versioned state at catalog version 7. Normalized execution records retain identity, source or command, state, result evidence, 1 MB stdout, 1 MB stderr, 256 KiB allowlisted VM diagnostics, truncation flags, and timestamps. Migration v7 backfills historical execution events, and partial records survive failure, cancellation, and restart recovery.
