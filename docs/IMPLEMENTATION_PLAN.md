...
- `vault debug-session` opens the catalog read-only and reconstructs one isolated session into a fresh owner-only temporary directory. It verifies immutable artifacts and a stable content-addressed workspace manifest without adding a daemon method or persistent product state.
