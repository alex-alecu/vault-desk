...
| Knowledge Bundle archive reader | Adopt tar-stream for deterministic, uncompressed M5 fixture bundles | Streaming and minimal, but hostile archive inspection stays inside the no-NIC guest. It is not a trust boundary. Compression is deferred so M0 does not stabilize an experimental Node zstd dependency. |
| Knowledge Bundle trust metadata | Adopt tuf-js direction plus Node Ed25519 verification | TUF supplies rollback/expiry/version metadata semantics; Node crypto supplies the signature primitive. Core will own policy and installed state. M0 validates library load and offline primitives, not the M10 import workflow. |
...
- Compressed Knowledge Bundle transport is deferred to M10. M5 consumes unpacked immutable fixture bundles, so selecting compression now would add an unused dependency and prematurely stabilize a transport.
