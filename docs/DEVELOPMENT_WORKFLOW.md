...
This document is the canonical implementation and contribution workflow for Vault Desk. M0, M1, and M2 are complete; M3 is not authorized. [AGENTS.md](../AGENTS.md) remains authoritative, followed by accepted architecture decision records, [IMPLEMENTATION_PLAN.md](IMPLEMENTATION_PLAN.md), and this workflow.
...
Roadmap presence is not authorization. M2 completed on 2026-07-20; M3 begins only on a new explicit owner request. Implementation outside the active scope must be converted into an issue, design note, or plan rather than code.
